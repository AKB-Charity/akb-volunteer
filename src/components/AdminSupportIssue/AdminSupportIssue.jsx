...
const AdminSupportIssue = () => {
  ...
  const handleIssueClick = (issue) => {
    navigate(`/admin/contact-support-issue-two/${issue.id}`);
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      {/* Main Content */}
      <main className="flex-grow flex flex-col items-center pt-20 pb-16 mb-5">
        {/* Pending Support Issues Section */}
        <div className="bg-gray-200 rounded-lg shadow-lg w-full md:w-3/4 p-8">
          ...
          <div className="space-y-4">
            {issues.map((issue) => (
              <div
                key={issue.id}
                className="flex justify-between items-center bg-gray-50 hover:bg-gray-100 p-6 rounded-md shadow-sm cursor-pointer"
...
export default AdminSupportIssue;
