...
const AdminSupportIssueTwo = () => {
  ...
  const handleYesClick = async () => {
    try {
      ...
      setShowPopup(false);
      navigate("/admin/contact-support-issues");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      ...
      {/* Main Content */}
      <main className="flex-1 w-full max-w-md lg:max-w-7xl mx-auto mt-16 p-6 lg:p-12 mb-10">
        {/* Card */}
        <div className="bg-white rounded-lg shadow-md p-6 lg:p-12">
          {/* ID Display */}
          ...
          <div className="mb-4">
            ...
            <input
              ...
              readOnly
              className="w-full p-2 lg:p-4 border border-gray-300 rounded bg-gray-100 text-gray-700"
            />
          </div>
            ...
            <div className="flex space-x-4">
              {Array.isArray(issue.documents) &&
                issue.documents.map((doc, index) => (
                  <div
                    key={index}
                    className="w-20 h-20 lg:w-32 lg:h-32 bg-gray-300 rounded"
                  ></div>
                ))}
            </div>
          </div>
          ...
          <div className="flex justify-between">
            <button
              className="flex-1 mr-2 bg-green-500 text-white py-2 lg:py-4 rounded font-bold hover:bg-green-600"
              onClick={() => window.open(issue.whatsapp_url, "_blank")}
            >
              Chat on WhatsApp
            </button>
            <button
              className="flex-1 ml-2 bg-red-500 text-white py-2 lg:py-4 rounded font-bold hover:bg-red-600"
...
export default AdminSupportIssueTwo;
